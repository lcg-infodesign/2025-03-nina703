...
// Mappa dei glifi per tipo di vulcano
const GLYPH_TYPES = {
  'Stratovolcano': 'circle',
  'Shield': 'triangle',
  'Volcanic field': 'square',
  'Caldera': 'pentagon',
  'Complex': 'concentric',
  'Submarine': 'cross',
  'Pyroclastic cone': 'rectangle',
  'Lava dome': 'star',
  'Crater': 'ellipse'
};
...
function prepareData() {
  ...
  for (let i = 0; i < table.getRowCount(); i++) {
    let row = table.getRow(i);
    let elevation = parseFloat(row.getString("Elevation (m)"));
    let volcanoType = row.getString("Type") || row.getString("Primary Type") || "";
    let lat = parseFloat(row.getString("Latitude"));
    let lon = parseFloat(row.getString("Longitude"));
...
function draw() {
  // Sfondo elegante: gradiente da blu-grigio a grigio chiaro
...
function drawVolcanoCard(v) {
  ...
  let type = GLYPH_TYPES[v.type] || 'circle';
  switch (type) {
    case 'circle':
      ellipse(0, 0, size, size);
      break;
  ...
  printDetail("Number", v.data.number)
  printDetail("Country", v.data.country);
  printDetail("Type", v.data.type);
  printDetail("Status", v.data.status);
  printDetail("Elevation", v.data.elevation ? v.data.elevation + " m" : "");
  printDetail("Last Eruption", v.data.lastEruption);
